...
import numpy as np
import torch
from rflib.parallel import collate, scatter
...
def init_detector(config, checkpoint=None, device='cuda:0', cfg_options=None):
    ...
    Args:
        config (str or :obj:`rflib.Config`): Config file path or the config
            object.
    ...
    if isinstance(config, str):
        config = rflib.Config.fromfile(config)
    elif not isinstance(config, rflib.Config):
...
class LoadImage:
    ...
    def __call__(self, results):
        ...
        warnings.warn('`LoadImage` is deprecated and will be removed in '
                      ...
                      'from `rfvision.datasets.pipelines.` instead.')
