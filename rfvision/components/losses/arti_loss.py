...
class ArtiMIoULoss(nn.Module):
    def __init__(self, contain_bg=True):
        super(ArtiMIoULoss, self).__init__()
...
@LOSSES.register_module()
class ArtiVECTLoss(nn.Module):
    def __init__(self, TYPE_L='L2',
                 MULTI_HEAD=False, SELF_SU=False):
        super(ArtiVECTLoss, self).__init__()
        self.type = TYPE_L
        self.multi_head = MULTI_HEAD
        self.self_su = SELF_SU
    ...
    def forward(self, vect, vect_gt, confidence=None, mask_array=None):
        if self.multi_head:
            loss_vect = 0
            vect_splits = torch.split(vect, split_size_or_sections=int(vect.shape[2] / self.num_parts), dim=2)
            mask_splits = torch.split(mask_array, split_size_or_sections=int(mask_array.shape[2] / self.num_parts), dim=2)
