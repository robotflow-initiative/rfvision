...
from rflib.ops import (PointFPModule, Points_Sampler, QueryAndGroup,
                         gather_points)
...
class LocalTransformer(nn.Module):
    ...
    def __init__(self, npoint, radius, nsample, dim_feature, dim_out, nhead=4, num_layers=2, norm_cfg=dict(type='BN2d'), ratio=1, drop=0.0, prenorm=True):
        ...
        self.grouper = QueryAndGroup(
            self.radius, self.nsample, use_xyz=False, return_grouped_xyz=True, normalize_xyz=False)

        self.pe = nn.Sequential(
            ConvModule(3, self.nc_in // 2, 1, norm_cfg=norm_cfg),
            ConvModule(self.nc_in // 2, self.nc_in,
                       1, act_cfg=None, norm_cfg=None)
        )
        ...
        self.chunk = nn.TransformerEncoder(
            ...
            LinformerEncoderLayer(src_len=nsample, ratio=ratio, d_model=self.nc_in,
                                  nhead=nhead, dropout=drop, dim_feedforward=2 * self.nc_in),
    ...
    def forward(self, xyz, features):
        ...
        input_features = input_features.permute(
            0, 2, 1, 3).reshape(-1, D, ns).permute(2, 0, 1)
        transformed_feats = self.chunk(input_features).permute(
            1, 2, 0).reshape(B, np, D, ns).transpose(1, 2)
        output_features = F.max_pool2d(transformed_feats, kernel_size=[
                                       1, ns])  # (B, C, npoint)
...
class GlobalTransformer(nn.Module):
    ...
    def __init__(self, dim_feature, dim_out, nhead=4, num_layers=2, norm_cfg=dict(type='BN2d'), ratio=1, src_pts=2048, drop=0.0, prenorm=True):
        ...
        self.chunk = nn.TransformerEncoder(
            BSC_Encoder(d_model=self.nc_in, dim_feedforward=2 * self.nc_in, dropout=drop, nhead=nhead) if ratio == 1 else
            LinformerEncoderLayer(src_len=src_pts, ratio=ratio, d_model=self.nc_in,
                                  nhead=nhead, dropout=drop, dim_feedforward=2 * self.nc_in),
            num_layers=num_layers)
...
class LocalGlobalTransformer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, nhead=4, num_layers=2, norm_cfg=dict(type='BN2d'), ratio=1, mem_pts=20000, tgt_pts=2048, drop=0.0, dim_feature=64, prenorm=True):
        ...
        self.chunk = nn.TransformerDecoder(
            ...
            LinformerDecoderLayer(tgt_len=tgt_pts, mem_len=mem_pts, ratio=ratio,
                                  d_model=self.nc_in, nhead=nhead, dropout=drop, dim_feedforward=2 * self.nc_in),
            num_layers=num_layers)
...
class BasicDownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.local_chunk = LocalTransformer(
            npoint, radius, nsample, dim_feature, dim_hid, nhead, num_layers, norm_cfg, enc_ratio, local_drop, prenorm)
        self.global_chunk = GlobalTransformer(
            dim_hid, dim_out, nhead, num_layers, norm_cfg, enc_ratio, npoint, global_drop, prenorm)
        if use_decoder:
            self.combine_chunk = LocalGlobalTransformer(
                dim_hid, dim_hid, nhead, 1, norm_cfg, ratio, mem_pts, npoint, decoder_drop, dim_feature, prenorm)
    ...
    def forward(self, xyz, features):
        ...
        if self.use_decoder:
            combined = self.combine_chunk(
                new_xyz, xyz, local_features, features)
...
class Pointformer(nn.Module):
    ...
    def __init__(self,
        ...
        # TODO: add rgb option
        self.feature_conv = ConvModule(4, bc, 1, norm_cfg=None, act_cfg=None)

        mem_pts = cloud_points

        for i, (np, r, ns, ratio, use) in enumerate(zip(num_points, radius, num_samples, ratios, use_decoder)):
            if i < self.num_sa - 1:
                self.SA_modules.append(BasicDownBlock(np, r, ns, bc, bc, bc * 2, num_heads, num_layers,
                                       norm_cfg, ratio, mem_pts, use_lin_enc, use, local_drop, global_drop, decoder_drop, prenorm))
                self.nc_outs.append(bc * 2)
            else:
                self.SA_modules.append(BasicDownBlock(np, r, ns, bc, bc, bc, num_heads, num_layers, norm_cfg,
                                       ratio, mem_pts, use_lin_enc, use, local_drop, global_drop, decoder_drop, prenorm))
        ...
        self.FP_modules = nn.ModuleList(
            [
                PointFPModule([self.nc_outs[-1] + self.nc_outs[-2],
                              fp_channels[0][0], fp_channels[0][1]], norm_cfg=norm_cfg),
                PointFPModule([self.nc_outs[-3] + fp_channels[0][1],
                              fp_channels[1][0], fp_channels[1][1]], norm_cfg=norm_cfg)
            ]
        )
    ...
    def forward(self, points):
        ...
        features = self.feature_conv(
            points[..., 3:].transpose(1, 2).unsqueeze(-1)).squeeze(-1)
        batch, num_points = xyz.shape[:2]
        indices = xyz.new_tensor(range(num_points)).unsqueeze(
            0).repeat(batch, 1).long()
        ...
        ret = dict(fp_xyz=fp_xyz, fp_features=fp_features,
