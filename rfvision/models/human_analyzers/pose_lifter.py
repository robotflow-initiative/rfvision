...
from rfvision.core import imshow_bboxes, imshow_keypoints, imshow_keypoints_3d
from .. import builder
from ..builder import HUMAN_ANALYZERS
...
class PoseLifter(BasePose):
    ...
    def show_result(self,
        ...
        if img is not None:
            ...
            if len(pose_input_2d) > 0:
                imshow_keypoints(
                    ...
                    radius=radius,
                    thickness=thickness)
            img = rflib.imrescale(img, scale=vis_height / img.shape[0])
        ...
        if show:
            rflib.visualization.imshow(img_vis, win_name, wait_time)

        if out_file is not None:
            rflib.imwrite(img_vis, out_file)
