...
class TouchEncoder(BasePose):
	...
	def get_pred(self,
		...
		pos = ref_frame['pos'].to(pred_depth.device).view(batch_size, -1)
		rot_m = ref_frame['rot_M'].to(pred_depth.device).view(-1, 3, 3)
		...
		for points, useful in zip(projected_depths, useful_points):
			...
			if points.shape[0] == 0:
				if producing_sheet:
					pred_points.append(torch.zeros((self.num_samples, 3)).cuda())
					continue
				else:
					points = orig_points
	...
	def forward_train(self,
					  ...
					  pos,
					  **kwargs):
		ref_frame = {'rot': rot, 'rot_M':rot_M, 'pos': pos}
		pred_depth, pred_points = self.get_pred(sim_touch, ref_frame, empty=empty)
		loss_dis = self.loss_coeff * point_loss(gt_points, pred_points)
	...
	def forward_test(self,
					 ...
					 pos,
					 **kwargs):
		losses = self.forward_train(gt_points,
								    sim_touch,
									empty,
									rot,
									rot_M,
