...
def show_result(points,
    ...
    rflib.mkdir_or_exist(result_path)
...
def show_multi_modality_result(img,
    ...
    if show:
        ...
        rflib.imshow(show_img, win_name='project_bbox3d_img', wait_time=0)

    if img is not None:
        rflib.imwrite(img, osp.join(result_path, f'{filename}_img.png'))

    if gt_bboxes is not None:
        gt_img = draw_bbox(
            gt_bboxes, img, proj_mat, img_metas, color=gt_bbox_color)
        rflib.imwrite(gt_img, osp.join(result_path, f'{filename}_gt.png'))

    if pred_bboxes is not None:
        pred_img = draw_bbox(
            pred_bboxes, img, proj_mat, img_metas, color=pred_bbox_color)
        rflib.imwrite(pred_img, osp.join(result_path, f'{filename}_pred.png'))
