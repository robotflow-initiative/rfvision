...
from rflib.visualization.color import color_val
...
def imshow_bboxes(img,
    ...
    # adapt to rflib.imshow_bboxes input format
    bboxes = np.split(
        bboxes, bboxes.shape[0], axis=0) if bboxes.shape[0] > 0 else []
    if not isinstance(colors, list):
        colors = [colors for _ in range(len(bboxes))]
    colors = [rflib.color_val(c) for c in colors]
    assert len(bboxes) == len(colors)

    img = rflib.imshow_bboxes(
        img,
        bboxes,
        colors,
        top_k=-1,
    ...
    if labels is not None:
        ...
        for bbox, label, color in zip(bboxes, labels, colors):
            ...
            cv2.putText(img, label, (text_x1, text_y2 - text_baseline),
                        ...
                        rflib.color_val(text_color), thickness)

    if show:
        rflib.imshow(img, win_name, wait_time)
...
def imshow_keypoints_3d(
    ...
    *,
    ...
    if show_img:
        img = rflib.imread(img, channel_order='bgr')
        img = rflib.bgr2rgb(img)
