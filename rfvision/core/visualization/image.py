...
def imshow_det_bboxes(img,
    ...
    img = rflib.imread(img).astype(np.uint8)
    ...
    if labels.shape[0] > 0:
        if mask_color is None:
            ...
            # specify  color
            mask_colors = [
                np.array(rflib.color_val(mask_color)[::-1], dtype=np.uint8)
            ] * (
                max(labels) + 1)
...
def imshow_gt_det_bboxes(img,
    ...
    if segm_result is not None and len(labels) > 0:  # non empty
        segms = rflib.concat_list(segm_result)
