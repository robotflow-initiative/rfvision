...
class Resize:
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert rflib.is_list_of(self.img_scale, tuple)
    ...
    def random_select(img_scales):
        ...
        assert rflib.is_list_of(img_scales, tuple)
    ...
    def random_sample(img_scales):
        ...
        assert rflib.is_list_of(img_scales, tuple) and len(img_scales) == 2
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = rflib.imrescale(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
                # the w_scale and h_scale has minor difference
                # a real fix should be done in the rflib.imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results[key].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img, w_scale, h_scale = rflib.imresize(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = rflib.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = rflib.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        if isinstance(flip_ratio, list):
            assert rflib.is_list_of(flip_ratio, float)
        ...
        if isinstance(direction, str):
            ...
            assert rflib.is_list_of(direction, str)
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                ...
                padded_img = rflib.impad_to_multiple(
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            results[key] = rflib.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        out_img = rflib.imresize(out_img, (int(out_img.shape[1] * jit_factor),
...
class LetterResize(object):
    ...
    def __call__(self, results):
        ...
        if gt_bboxes is not None:
            # bbox xyxy instead of xywh
            gt_bboxes[:, [0, 2]] = gt_bboxes[:, [0, 2]] * ratio[0] + left
            gt_bboxes[:, [1, 3]] = gt_bboxes[:, [1, 3]] * ratio[1] + top
        return results
