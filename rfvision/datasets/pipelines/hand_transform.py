...
class HandGenerateRelDepthTarget:
    ...
    def __call__(self, results):
        ...
        root_depth_bound = cfg.get('root_depth_bound', None)

        # tycoer
        if root_depth_bound is not None:
            if isinstance(root_depth_bound, (int, float)):
                root_depth_bound = [root_depth_bound]
            if len(root_depth_bound) == 1:
                target = (rel_root_depth / root_depth_bound + 0.5) * D
            elif len(root_depth_bound) == 2:
                assert root_depth_bound[0] < root_depth_bound[1]
                # normalize : (x - min) / (max - min)
                target = (rel_root_depth - root_depth_bound[0]) / (root_depth_bound[1] - root_depth_bound[0]) * D
