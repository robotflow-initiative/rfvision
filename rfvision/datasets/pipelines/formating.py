...
class ToDataContainer:
    """Convert results to :obj:`rflib.DataContainer` by given fields.

    Args:
        fields (Sequence[dict]): Each field is a dict like
            ``dict(key='xxx', **kwargs)``. The ``key`` in result will
            be converted to :obj:`rflib.DataContainer` with ``**kwargs``.
...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        for key in ['proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels', 'gt_coefs']:
...
class Collect:
    ...
    Args:
        ...
        meta_keys (Sequence[str], optional): Meta keys to be converted to
            ``rflib.DataContainer`` and collected in ``data[img_metas]``.
    ...
    def __call__(self, results):
        ...
        will be converted to :obj:rflib.DataContainer.
