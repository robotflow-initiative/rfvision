...
from rfvision.core.camera import CAMERAS
from rfvision.core.post_processing_pose import fliplr_regression
...
class NormalizeJointCoordinate:
    ...
    Args:
        ...
        norm_param_file (str): Optionally load a dict containing `mean` and
            `std` from a file using `rflib.load`.
    Required keys:
        item
    Modified keys:
        item
    ...
    def __init__(self, item, mean=None, std=None, norm_param_file=None):
        ...
        if norm_param_file is not None:
            norm_param = rflib.load(norm_param_file)
...
class Generate3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        heatmap3d_depth_bound = cfg.get('heatmap3d_depth_bound', None)
        ######################################
        ...
        # tycoer
        if heatmap3d_depth_bound is not None:
            if isinstance(heatmap3d_depth_bound, (int, float)):
                heatmap3d_depth_bound = [heatmap3d_depth_bound]

            if len(heatmap3d_depth_bound) == 1:
                mu_z = (joints_3d[:, 2] / heatmap3d_depth_bound[0] + 0.5) * D
            elif len(heatmap3d_depth_bound) == 2:
                assert heatmap3d_depth_bound[0] < heatmap3d_depth_bound[1]
                # normalize : (x - min) / (max - min)
                mu_z = (joints_3d[:, 2] - heatmap3d_depth_bound[0]) / (heatmap3d_depth_bound[1] - heatmap3d_depth_bound[0]) * D
        else:
            mu_z = (joints_3d[:, 2])
