...
class RHDDataset(Dataset):
    ...
    def __init__(self,
        ...
        if self.split == 'train':
            ...
            self.annos = rflib.load(os.path.join(self.data_root, 'training', 'anno_training.pickle'))
        else:
            self.split_root = os.path.join(os.path.join(self.data_root, 'evaluation'))
            self.annos = rflib.load(os.path.join(self.data_root, 'evaluation', 'anno_training.pickle'))
    ...
    def __getitem__(self, index):
        rgb = rflib.imread(os.path.join(self.split_root, 'color', '%05d' % index + '.png'))
        results = {
                   'img': rgb,
