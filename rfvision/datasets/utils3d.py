...
# yapf: disable
from rfvision.datasets.pipelines import (Collect3D, DefaultFormatBundle3D,
                                        LoadAnnotations3D,
...
from rfvision.datasets.pipelines import LoadImageFromFile
import rflib
...
def get_loading_pipeline_3d(pipeline):
    ...
    for transform in pipeline:
        ...
        if is_loading is None:  # MultiScaleFlipAug3D
            ...
            loading_pipeline.extend(get_loading_pipeline_3d(inner_pipeline))
        elif is_loading:
...
def extract_result_dict(results, key):
    ...
    if isinstance(data, (list, tuple)):
        data = data[0]
    if isinstance(data, rflib.parallel.DataContainer):
        data = data._data
    return data
