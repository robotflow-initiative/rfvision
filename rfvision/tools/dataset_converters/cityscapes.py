...
def collect_annotations(files, nproc=1):
    ...
    if nproc > 1:
        images = rflib.track_parallel_progress(
            load_img_info, files, nproc=nproc)
    else:
        images = rflib.track_progress(load_img_info, files)
...
def load_img_info(files):
    ...
    inst_img = rflib.imread(inst_file, 'unchanged')
...
def cvt_annotations(image_infos, out_json_name):
    ...
    rflib.dump(out_json, out_json_name)
    return out_json
