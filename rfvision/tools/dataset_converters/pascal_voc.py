...
from rfvision.core import voc_classes
...
def cvt_annotations(devkit_path, years, split, out_file):
    ...
    for year in years:
        ...
        if not osp.isfile(filelist):
            ...
            return
        img_names = rflib.list_from_file(filelist)
        xml_paths = [
        ...
        part_annotations = rflib.track_progress(parse_xml,
                                               list(zip(xml_paths, img_paths)))
        annotations.extend(part_annotations)
    if out_file.endswith('json'):
        annotations = cvt_to_coco_json(annotations)
    rflib.dump(annotations, out_file)
    return annotations
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert PASCAL VOC annotations to rfvision format')
