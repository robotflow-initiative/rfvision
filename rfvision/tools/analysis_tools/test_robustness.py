...
from rfvision import datasets
from rfvision.apis import multi_gpu_test, set_random_seed, single_gpu_test
...
def voc_eval_with_return(result_file,
                         ...
                         logger='print',
                         only_ap=True):
    det_results = rflib.load(result_file)
...
def parse_args():
    parser = argparse.ArgumentParser(description='RFVision test detector')
...
def main():
    ...
    cfg = rflib.Config.fromfile(args.config)
    ...
    for corr_i, corruption in enumerate(corruptions):
        ...
        for sev_i, corruption_severity in enumerate(args.severities):
            ...
            if args.out and rank == 0:
                ...
                rflib.dump(outputs, args.out)
                eval_types = args.eval
                if cfg.dataset_type == 'VOCDataset':
                    if eval_types:
                        for eval_type in eval_types:
                            if eval_type == 'bbox':
                                test_dataset = rflib.runner.obj_from_dict(
                ...
                rflib.dump(aggregated_results, eval_results_filename)
