...
def test_bbox_head_loss():
    ...
    target_cfg = rflib.Config(dict(pos_weight=1))
...
def test_refine_boxes():
    ...
    ``rfvision.models.bbox_heads.bbox_head.BBoxHead.refine_boxes`` but checks for
...
def _demodata_refine_boxes(n_roi, n_img, rng=0):
    ...
    ``rfvision.models.bbox_heads.bbox_head.BBoxHead.refine_boxes`` method."""
    import numpy as np
    from rfvision.core.bbox.demodata import random_boxes
    from rfvision.core.bbox.demodata import ensure_rng
    try:
        import kwarray
    except ImportError:
        import pytest
