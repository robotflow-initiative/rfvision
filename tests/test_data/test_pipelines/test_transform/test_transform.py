...
def test_flip():
    ...
    if results['flip']:
        assert np.array_equal(
            rflib.imflip(original_img, results['flip_direction']),
            results['img'])
    else:
...
def test_multi_scale_flip_aug():
    ...
    results = dict(
        img_prefix=osp.join(osp.dirname(__file__), '../../../data'),
        img_info=dict(filename='color.jpg'))
    load_cfg, multi_scale_cfg = rflib.Config.fromfile(
    ...
    assert isinstance(results['img_metas'][0], rflib.parallel.DataContainer)
